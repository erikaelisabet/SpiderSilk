...
row_anno <- rowAnnotation(Family = df_sub$Family, col= list(Family= all_colors), Genus = anno_text(df_sub$Genus), Spicies = anno_text(df_sub$species_x), ID = anno_text(df_sub$ID))
...
clu3 <- draw(heatmap_object + row_anno)
plot(clu3)
...
clu4 <- draw(heatmap_object + row_anno)
plot(clu4)
