...
filled_df_non_spidroin = full_df_non_spidroin.fillna({'Group': 'NaN', 'geneName': 'NaN', 'product': 'NaN', 'lascID': 'NaN'})
filled_df_spidroin = full_df_spidroin.fillna({'Group': 'NaN', 'geneName': 'NaN', 'product': 'NaN', 'lascID': 'NaN'})

pivot_df_non_spidroin = filled_df_non_spidroin.pivot_table(
    index=['ID', 'idv_id', 'DRR', 'species', 'Genus', 'Family'], 
    columns=['Orthogroup', 'Group', "geneName", "product", "lascID"], 
    values='Tpm', 
    aggfunc='max'
).fillna(0)
...
pivot_df_spidroin = full_df_spidroin.pivot_table(index=['ID', 'idv_id', 'DRR', 'species', 'Genus', 'Family'], 
                               columns=['Orthogroup', 'Group', "geneName", "product", "lascID"], values='Cpm', aggfunc='max').fillna(0)
...
pivot_df_spidroin.to_csv("max_spidroins_cpm_annotated_lascID.csv", index=True)
pivot_df_non_spidroin.to_csv("max_non_spidroins_tpm_annotated_lascID.csv", index=True) 
pivot_df_all = pd.merge(pivot_df_non_spidroin, pivot_df_spidroin)
pivot_df_all.to_csv("max_all_annotated_lascID.csv", index=True) 