...
pivot_df_spidroin = full_df_spidroin.pivot_table(index=['ID', 'idv_id', 'DRR', 'species', 'Genus', 'Family'], 
                               columns=['Orthogroup', 'Group', "geneName", "product", "lascID"], values='Cpm', aggfunc='sum').fillna(0)
...
pivot_df_spidroin.to_csv("spidroins_cpm_annotated_lascID.csv", index=True)
pivot_df_non_spidroin.to_csv("non_spidroins_tpm_annotated_lascID.csv", index=True) 
pivot_df_all = pd.merge(pivot_df_non_spidroin, pivot_df_spidroin)
pivot_df_all.to_csv("all_annotated_lascID.csv", index=True) 
